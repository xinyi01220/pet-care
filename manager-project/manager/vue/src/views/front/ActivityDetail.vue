  ...
  <div class="main-content" style="width: 60%">
    <div class="card" style="margin-bottom: 10px">
      <div style="display: flex; grid-gap: 20px">
        <img :src="activity.img" alt="" style="width: 30%; height: 250px; border-radius: 5px">

        <div style="flex: 1; position: relative">
          <div style="font-weight: 550">发布组织: {{activity.departmentName}}</div>
          <div style="font-weight: bold; font-size: 24px; margin-bottom: 10px">{{ activity.name }}</div>
          <div style="color: #666; font-size: 13px; line-height: 22px; text-align: justify; margin-bottom: 10px">{{ activity.description }}</div>
  ...
  methods: {
    cancel() {   // 单个删除
      this.$confirm('您确定取消报名吗？', '确认取消', {type: "warning"}).then(response => {
        this.$request.delete('/activitySign/delete/' + this.activityId).then(res => {
